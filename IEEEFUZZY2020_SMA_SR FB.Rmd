...
FB <- get.hist.quote(instrument="FB",start = dateStart, end=dateEnd,quote = c("AdjClose"),
                        retclass="zoo")
...
```{r}
priceFB=price$Adjusted.FB    # "AAPL","AMZN","FB","GOOG","VIX"
sma20<-SMA(priceFB, 20)
sma10<-SMA(priceFB, 10)            
DataSMAcross<-na.omit(as.data.frame(cbind(sma20, sma10)))
colnames(DataSMAcross)<-c("sma20",'sma10') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[3])) %>% dySeries('sma20', col = "blue") %>% dySeries('sma10', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-09-20', to = '2021-10-29', color = '#CCEBD6') %>%
  dyShading(from = '2021-08-23', to = '2021-09-20', color = '#FFE6E6')
...
algo.ret.test<-c()
for (j in 1:length(nS)){
algo.ret.test[[j]]<-trading.strategy (price[, 3], return [, 3], nL = nL, nS = j)
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$Adjusted.FB[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-1.1, 0.9))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
for(i in 1:step){
  set.seed(123+i)
  nvf[[i]]<-neuro.vol(as.numeric(return$Adjusted.FB[i:(N-step+i)]))
}
```
