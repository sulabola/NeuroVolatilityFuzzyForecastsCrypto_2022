...
```{r}
priceBTC=price$BTC.USD.Adjusted
sma30<-SMA(priceBTC, 30)
sma10<-SMA(priceBTC, 10)
DataSMAcross<-na.omit(as.data.frame(cbind(sma30, sma10)))
colnames(DataSMAcross)<-c("sma30",'sma10') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[1])) %>% dySeries('sma30', col = "blue") %>% dySeries('sma10', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-9-12', to = '2021-10-05', color = '#CCEBD6') %>%
  dyShading(from = '2021-10-05', to = '2021-11-19', color = '#FFE6E6')
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$BTC.USD.Adjusted[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-1.0, 3.0))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
```{r}
nvf<-c()
tempdata=na.omit(return$BTC.USD.Adjusted)
