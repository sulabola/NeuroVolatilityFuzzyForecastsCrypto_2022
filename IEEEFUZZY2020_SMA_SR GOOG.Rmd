...
```{r}
priceGOOG=price$Adjusted.GOOG    # "AAPL","AMZN","FB","GOOG","VIX"
sma20<-SMA(priceGOOG, 20)
sma10<-SMA(priceGOOG, 10)            
DataSMAcross<-na.omit(as.data.frame(cbind(sma20, sma10)))
colnames(DataSMAcross)<-c("sma20",'sma10') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[4])) %>% dySeries('sma20', col = "blue") %>% dySeries('sma10', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-09-19', to = '2021-10-16', color = '#CCEBD6') %>%
  dyShading(from = '2021-10-16', to = '2021-11-29', color = '#FFE6E6')
...
algo.ret.test<-c()
for (j in 1:length(nS)){
algo.ret.test[[j]]<-trading.strategy (price[, 4], return [, 4], nL = nL, nS = j)
...
plot(sharpe.ratio[, j], type = "l", main=paste('Rolling DD-EWMA Daily SR:', main.names[j]), ylab="Daily DD-EWMA SR", lwd=1.5, col="black", , ylim = c(-0.6, 1.0))
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$Adjusted.GOOG[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-4.0, 1.5))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
for(i in 1:step){
  set.seed(123+i)
  nvf[[i]]<-neuro.vol(as.numeric(return$Adjusted.GOOG[i:(N-step+i)]))
}
```
