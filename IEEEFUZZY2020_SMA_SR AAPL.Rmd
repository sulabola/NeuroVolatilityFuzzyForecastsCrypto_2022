...
dateStart = "2017-01-01"; dateEnd = "2021-12-31"
...
main.names<-c("AAPL","AMZN","FB","GOOG","VIX")
...
#l can be changed to 1, 2, 3, 4, 5 for "AAPL","AMZN","FB","GOOG","VIX"
l <- 1
nL <- 20
nS<-seq(1, nL, 1)
algo.ret.test<-c()
...
nL<-20 #clong-term window size
nS<-10 #cshot-term window size      
algo.ret<-c()
for (j in 1:number){
...
```{r}
priceAAPL=price$Adjusted.AAPL     # "AAPL","AMZN","FB","GOOG","VIX"
sma20<-SMA(priceAAPL, 20)
sma10<-SMA(priceAAPL, 10)            
DataSMAcross<-na.omit(as.data.frame(cbind(sma20, sma10)))
colnames(DataSMAcross)<-c("sma20",'sma10') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[1])) %>% dySeries('sma20', col = "blue") %>% dySeries('sma10', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-09-18', to = '2021-10-19', color = '#CCEBD6') %>%
  dyShading(from = '2021-08-12', to = '2021-09-18', color = '#FFE6E6')
                                # main.names[i] i=1("AAPL"),2("AMZN"),3("FB"),4("GOOG"),5("VIX")
```
...
algo.ret.test<-c()
for (j in 1:length(nS)){
algo.ret.test[[j]]<-trading.strategy (price[, 1], return [, 1], nL = nL, nS = j)
}                                   # price/return[,i] i=1("AAPL"),2("AMZN"),3("FB"),4("GOOG"),5("VIX")
algo.ret.test<-as.data.frame(do.call(cbind, algo.ret.test))
...
for(i in 1:941){
    result<-dd(algo.ret.t.test[i:(60+i-1), j])
    vol<-result[1]
    sharpe.ratio[i]<-(mean(algo.ret.t.test[i:(60+i-1), j])-0.02/252)/vol
...
rmse.algo<-matrix(0, nrow=941, ncol=number)
sharpe.ratio<-matrix(0, nrow=941, ncol=number)
for(j in 1:number){
  ptm <- proc.time()
  for(i in 1:941){
    result<-dd(algo.ret.t[i:(60+i-1), j])
    vol<-result[1]
    sharpe.ratio[i, j]<-(mean(algo.ret.t[i:(60+i-1), j])-0.02/252)/vol
...
j <- 1
Y = rep(sr.daily[j], 941)
Zl1 <- rep(sr.daily[j] - qnorm(1-.05/2)*sr.daily.sd[j], 941)
Zu1 <- rep(sr.daily[j] + qnorm(1-.05/2)*sr.daily.sd[j], 941)
Zl2 <- rep(sr.daily[j] - qnorm(1-.01/2)*sr.daily.sd[j], 941)
Zu2 <- rep(sr.daily[j] + qnorm(1-.01/2)*sr.daily.sd[j], 941)
plot(sharpe.ratio[, j], type = "l", main=paste('Rolling DD-EWMA Daily SR:', main.names[j]), ylab="Daily DD-EWMA SR", lwd=1.5, col="black", ylim = c(-1.30, 0.8))
...
# step = 60 rolling DDVF volatlity forecasts
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$Adjusted.AAPL[i:(N-step+i)]))
}
```
...
alpha_cuts = function(a,alpha,m,n){
  return(sqrt(252)*c(a[1]+alpha^{1/m}*(a[2]-a[1]), a[4]-alpha^{1/n}*(a[4]-a[3])))
}
```
...
#j can be changed to 1, 2, 3, 4, 5 for "AAPL","AMZN","FB","GOOG","VIX"
set.seed(123)
j<-1
ptm <- proc.time()
sharpe.ratio<-matrix(0, nrow=941, ncol=1)
for(i in 1:941){
    vol<-neuro.vol(algo.ret.t[i:(60+i-1), j])
    sharpe.ratio[i]<-(mean(algo.ret.t[i:(60+i-1), j])-0.02/252)/vol
}
...
alpha_cut_l_0.01 <- sr.daily - qnorm(1-.01/2)*sr.daily.sd
alpha_cut_u_0.01 <- sr.daily + qnorm(1-.01/2)*sr.daily.sd
temp <- cbind (sr.daily,sqrt(252)*sr.daily, sqrt(252)*alpha_cut_l_0.05, sqrt(252)*alpha_cut_u_0.05, sqrt(252)*alpha_cut_l_0.01, sqrt(252)*alpha_cut_u_0.01)
xtable(temp, digit = 3)
```
...
Y = rep(sr.daily, 941)
Zl1 <- rep(alpha_cut_l_0.05, 941)
Zu1 <- rep(alpha_cut_u_0.05, 941)
Zl2 <- rep(alpha_cut_l_0.01, 941)
Zu2 <- rep(alpha_cut_u_0.01, 941)
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-1.2, 6.5))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
# step = 60 rolling NVF volatlity forecasts
...
for(i in 1:step){
  set.seed(123+i)
  nvf[[i]]<-neuro.vol(as.numeric(return$Adjusted.AAPL[i:(N-step+i)]))
}
```
