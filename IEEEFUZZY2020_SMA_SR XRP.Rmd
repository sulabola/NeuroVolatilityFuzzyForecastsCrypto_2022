...
```{r}
priceXRP=price$XRP.USD.Adjusted     # "BTC","ETH","BNB","XRP","DOGE","ADA"
sma30<-SMA(priceXRP, 30)
sma15<-SMA(priceXRP, 15)            # "BTC"&"ETH"=10,"BNB"&"XRP"=15,"DOGE"&"ADA"=20
DataSMAcross<-na.omit(as.data.frame(cbind(sma30, sma15)))
colnames(DataSMAcross)<-c("sma30",'sma15') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[4])) %>% dySeries('sma30', col = "blue") %>% dySeries('sma15', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-09-14', to = '2021-10-10', color = '#CCEBD6') %>%
  dyShading(from = '2021-10-10', to = '2021-10-30', color = '#FFE6E6')
...
plot(sharpe.ratio[, j], type = "l", main=paste('Rolling DD-EWMA Daily SR:', main.names[j]), ylab="Daily DD-EWMA SR", lwd=1.5, col="black", ylim = c(-0.4, 0.4))
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$XRP.USD.Adjusted[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-2.0, 2.0))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
```{r}
nvf<-c()
tempdata=na.omit(return$XRP.USD.Adjusted)
