...
```{r}
priceETH=price$ETH.USD.Adjusted     # "BTC","ETH","BNB","XRP","DOGE","ADA"
sma30<-SMA(priceETH, 30)
sma10<-SMA(priceETH, 10)            # "BTC"&"ETH"=10,"BNB"&"XRP"=15,"DOGE"&"ADA"=20
DataSMAcross<-na.omit(as.data.frame(cbind(sma30, sma10)))
colnames(DataSMAcross)<-c("sma30",'sma10') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[2])) %>% dySeries('sma30', col = "blue") %>% dySeries('sma10', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-9-16', to = '2021-10-06', color = '#CCEBD6') %>%
  dyShading(from = '2021-10-06', to = '2021-11-21', color = '#FFE6E6')
...
plot(sharpe.ratio[, j], type = "l", main=paste('Rolling DD-EWMA Daily SR:', main.names[j]), ylab="Daily DD-EWMA SR", lwd=1.5, col="black", ylim = c(-0.4, 0.5))
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$ETH.USD.Adjusted[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-1.5, 10.0))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
```{r}
nvf<-c()
tempdata=na.omit(return$ETH.USD.Adjusted)
