...
```{r}
priceDOGE=price$DOGE.USD.Adjusted     # "BTC","ETH","BNB","XRP","DOGE","ADA"
sma30<-SMA(priceDOGE, 30)
sma20<-SMA(priceDOGE, 20)            # "BTC"&"ETH"=10,"BNB"&"XRP"=15,"DOGE"&"ADA"=20
DataSMAcross<-na.omit(as.data.frame(cbind(sma30, sma20)))
colnames(DataSMAcross)<-c("sma30",'sma20') 
dygraph(DataSMAcross, main = paste('SMA:', main.names[5])) %>% dySeries('sma30', col = "blue") %>% dySeries('sma20', col = "red") %>% dyRangeSelector(height = 30) %>%
  dyShading(from = '2021-09-08', to = '2021-10-15', color = '#CCEBD6') %>%
  dyShading(from = '2021-08-06', to = '2021-09-08', color = '#FFE6E6')
...
plot(sharpe.ratio[, j], type = "l", main=paste('Rolling DD-EWMA Daily SR:', main.names[j]), ylab="Daily DD-EWMA SR", lwd=1.5, col="black", ylim = c(-0.4, 0.8))
...
for(i in 1:step){
  ddvf[[i]]<-dd(as.numeric(return$DOGE.USD.Adjusted[i:(N-step+i)]))
}
```
...
plot(sharpe.ratio, type = "l", main=paste('Rolling Neuro Daily SR:', main.names[j]), ylab="Daily Neuro SR", lwd=1.5, col="black", ylim = c(-1.70, 10.0))
lines(Y, lwd=1.5, col = "red")
lines(Zl1, lwd=1.5, col = "blue")
lines(Zu1, lwd=1.5, col = "blue")
lines(Zl2, lwd=1.5, col = "purple")
...
```{r}
nvf<-c()
tempdata=na.omit(return$DOGE.USD.Adjusted)
